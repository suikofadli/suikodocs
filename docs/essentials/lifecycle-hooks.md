...
Livewire menyediakan berbagai lifecycle hooks yang memungkinkan Anda untuk mengeksekusi kode pada titik-titik tertentu selama lifecycle komponen. Hook ini memungkinkan Anda untuk melakukan tindakan sebelum atau setelah event tertentu, seperti menginisialisasi komponen, memperbarui properties, atau merender template.
...
Dalam kelas PHP standar, constructor (`__construct()`) menerima parameter luar dan menginisialisasi state objek. Namun, di Livewire, Anda menggunakan metode `mount()` untuk menerima parameter dan menginisialisasi state komponen Anda.

Komponen Livewire tidak menggunakan `__construct()` karena komponen Livewire _dibangun kembali_ pada request jaringan berikutnya, dan kami hanya ingin menginisialisasi komponen sekali ketika pertama kali dibuat.

Berikut adalah contoh menggunakan metode `mount()` untuk menginisialisasi properties `name` dan `email` dari komponen `UpdateProfile`:
...
Seperti disebutkan sebelumnya, metode `mount()` menerima data yang dimasukkan ke dalam komponen sebagai parameter metode:
...
* [Menerima data dari komponen induk](/docs/nesting#passing-props-to-children)
* [Mengakses parameter route](/docs/components#accessing-route-parameters)

## Boot

Seb helpful `mount()`, ini hanya berjalan sekali per lifecycle komponen, dan Anda mungkin ingin menjalankan logika di awal setiap request ke server untuk komponen tertentu.

Untuk kasus ini, Livewire menyediakan metode `boot()` di mana Anda dapat menulis kode setup komponen yang Anda niatkan untuk dijalankan setiap kali kelas komponen di-boot: baik pada inisialisasi maupun pada request berikutnya.

Metode `boot()` dapat berguna untuk hal-hal seperti menginisialisasi protected properties, yang tidak dipertahankan antara request. Di bawah ini adalah contoh menginisialisasi protected property sebagai model Eloquent:
...
User sisi klien dapat memperbarui public properties dengan banyak cara berbeda, yang paling umum dengan memodifikasi input yang memiliki `wire:model` di atasnya.
...
Istilah "dehydrate" dan "hydrate" mengacu pada komponen Livewire yang diserialisasikan ke JSON untuk sisi klien dan kemudian di-unserialisasikan kembali ke objek PHP pada request berikutnya.
...
Mari kita lihat contoh yang menggunakan `mount()`, `hydrate()`, dan `dehydrate()` semua bersama-sama untuk mendukung penggunaan custom [data transfer object (DTO)](https://en.wikipedia.org/wiki/Data_transfer_object) alih-alih model Eloquent untuk menyimpan data post dalam komponen:
...
Sekarang, dari aksi dan tempat lain di dalam komponen Anda, Anda dapat mengakses objek `PostDto` alih-alih data primitif.
...
Jika Anda ingin mengaitkan ke proses rendering view Blade komponen, Anda dapat melakukannya menggunakan hook `rendering()` dan `rendered()`:
...
Traits adalah cara yang berguna untuk menggunakan kembali kode di seluruh komponen atau mengekstrak kode dari komponen tunggal ke file yang didedikasikan.

Untuk menghindari beberapa traits bentrok satu sama lain saat mendeklarasikan metode lifecycle hook, Livewire mendukung prefixing metode hook dengan nama _camelCased_ dari trait saat ini yang mendeklarasikannya.
...
Di bawah ini adalah contoh komponen yang mereferensikan trait yang disebut `HasPostForm`:
...
Objek form di Livewire mendukung hook pembaruan property. Hook ini bekerja mirip dengan [hook pembaruan komponen](#update), memungkinkan Anda melakukan tindakan ketika properties dalam objek form berubah.

Di bawah ini adalah contoh komponen yang menggunakan objek form `PostForm`:
